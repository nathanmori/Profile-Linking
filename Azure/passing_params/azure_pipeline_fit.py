...
def azureml_main(df):
    ...
    pipe = pipe_init()
    pipe.fit(df_clean.copy(), y)

    pipe_params = {}
    for step_name, step in pipe.named_steps.iteritems():
        pipe_params[step_name] = {}
        for key, val in step.__dict__.iteritems():
            pipe_params[step_name][key] = val

    #is pickle needed? might be able to df pipe_params dict as-is
    pkl_params = pickle.dumps(pipe_params)
    df_params = pd.DataFrame({'params': [pkl_params]})
