...
def azureml_main(df, df_params):
    ...
    pipe = pipe_init()
    pkl_params = df_params['params'][0]
    pipe_params = pickle.loads(pkl_params)
    for step_name, param_dict in pipe_params.iteritems():
        for key, val in param_dict.iteritems():
            pipe.named_steps[step_name].__dict__[key] = val
