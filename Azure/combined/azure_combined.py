...
class text_idf(UD_transform_class):
    ...
    def transform(self, (df_X, X_github, X_meetup)):
        ...
        X_github : numpy.array, None
            Github text vectors as array, if idf = no or both else None.

        X_meetup : numpy.array, None
            Meetup text vectors as array, if idf = no or both else None.

        X_github_tfidf : numpy.array, None
            Github text vectors with idf transform applied as array,
            if idf = yes or both else None.

        X_meetup_tfidf : numpy.array, None
            Meetup text vectors with idf transform applied as array,
            if idf = yes or both else None.
        """

        if self.params['idf'] == 'no':
            return (df_X, X_github, X_meetup, None,
                    None)
        ...
        return (df_X, X_github, X_meetup, X_github_tfidf.toarray(),
                X_meetup_tfidf.toarray())
...
class text_aggregate(UD_transform_class):
    ...
    def transform(self, (df_X, X_github, X_meetup, X_github_tfidf,
        ...
        X_github : numpy.array, None
            Github text vectors as array, if included.

        X_meetup : numpy.array, None
            Meetup text vectors as array, if included.
...
def azureml_main(df_train, df_test):

    df_clean_train = clean(df_train)
    df_clean_test = clean(df_test)

    y_train = df_clean_train.pop('match').values
    y_test = df_clean_test.pop('match').values
    ...
    pipe = Pipeline([('drop_github_meetup',
                       drop_github_meetup()),
                      ('dist_fill_missing',
                       dist_fill_missing()),
                      ('dist_diff',
                      ...
                      #('scaler',
                       #scaler())])
    pipe.set_params(**best_feats)
    pipe.fit(df_clean_train.copy(), y_train)

    df_trans_train = pipe.transform(df_clean_train.copy())
    df_trans_train['match'] = y_train

    df_trans_test = pipe.transform(df_clean_test.copy())
    df_trans_test['match'] = y_test
